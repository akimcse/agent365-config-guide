...
EXTRA = {
 ...
 80: """
      ...
      <div class="note">고급 헌팅에서 탐지 데이터에 <b>에이전트 인벤토리·구성 컨텍스트</b>를 붙이려면 <code>AgentsInfo</code> 테이블을 조인합니다. <code>AgentsInfo</code>는 <b>Microsoft Agent 365의 에이전트 목록·구성·소유자</b> 정보를 담는 테이블로, 에이전트별 <b>선언 도구·MCP 서버·권한·수명주기 상태</b>까지 조회할 수 있습니다. <span class="hl-note">참고: 이 테이블은 기존 <code>AIAgentsInfo</code>를 대체하며(<code>AIAgentsInfo</code>는 2026-07-01 폐기 예정), Agent 365 환경에서는 <code>AgentsInfo</code>를 사용합니다.</span><br><br><b>조인 키 — <code>EntraAgentID</code></b><br>탐지 증거 테이블 <code>AlertEvidence</code>에서 <code>EntityType == "AIAgent"</code> 행의 <code>AdditionalFields.AgentId</code>(Entra 에이전트 ID)를 꺼내, <code>AgentsInfo.EntraAgentID</code>와 조인합니다. 조인으로 보강되는 주요 필드는 다음과 같습니다.</div>
      <div class="tw tw-fields"><table><thead><tr><th>필드</th><th>의미</th></tr></thead><tbody><tr><td><code>Name</code></td><td>에이전트 표시 이름</td></tr><tr><td><code>Owners</code></td><td>소유자(Entra 개체 ID)</td></tr><tr><td><code>LifecycleStatus</code></td><td>운영 상태 — <code>Active</code>·<code>Blocked</code>·<code>Uninstalled</code>·<code>Deleted</code></td></tr><tr><td><code>DeclaredTools</code>·<code>McpServers</code></td><td>선언된 도구·연결된 MCP 서버</td></tr><tr><td><code>Permissions</code>·<code>PublishedStatus</code></td><td>요청·부여 권한과 게시 상태</td></tr></tbody></table></div>
      <div class="note">위 화면은 <b>XPIA(간접 프롬프트 인젝션) 경고</b>를 <code>EntraAgentID</code>로 조인해, 탐지된 에이전트(<code>foundry-ops</code>)의 <b>소유자·상태(Active)·도구(web_search)</b>를 함께 확인한 결과입니다.</div>
